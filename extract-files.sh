...
# Fingerprint  fringerprint.default.so => fringerprint.vendor.default.so 
"${PATCHELF}" --set-soname "fingerprint.vendor.default.so" "fingerprint.vendor.default.so"

# Goodix
sed -i "s|/system/etc/firmware|/vendor/firmware/gxf|g" "${DEVICE_BLOB_ROOT}"/vendor/lib64/gxfingerprint.default.so
